...
function update() {
  ...
  var dataTimeFiltered = filteredDataArray.filter(function(d) {
    // console.log(Number(d.date) >= sliderValues[0] && Number(d.date) <= sliderValues[1]);
    return (
      Number(d.date) >= sliderValues[0] && Number(d.date) <= sliderValues[1]
    );
  });
  ...
  function mousemove() {
    ...
    focus.attr(
      "transform",
      "translate(" + x(d.date) + "," + y(Number(d.totalSales)) + ")"
    );
    focus.select("text").text(function() {
      // console.log(d);
      return d3.format("$,")(Number(d.totalSales).toFixed(2));
    });
    focus.select(".x-hover-line").attr("y2", height - y(Number(d.totalSales)));
